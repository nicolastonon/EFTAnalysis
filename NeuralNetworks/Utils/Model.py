...
# INPUT LAYER
    ...
    if use_normInputLayer == True :
        # print('shifts', shifts); print('scales', scales)
        X = Lambda(normalize, arguments={'shift': shifts, 'scale': scales}, name="Feature_normalization",dtype='float64')(X)
...
# HIDDEN LAYERS
    ...
    for iLayer in range(nHiddenLayers):
        ...
        # Dense hidden layers
        X = Dense(nNeurons, activation=activ_tmp if activ_tmp not in ['lrelu','prelu'] else None, activity_regularizer=reg, kernel_initializer=kernInit)(X)
