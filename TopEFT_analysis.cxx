...
//  ####  #    # #    # #      ###### ######    ######  ####   ####  #
    	...
    	for(int isample=0; isample<sample_list.size(); isample++)
    	{
    		...
    		for(int itree=0; itree<systTree_list.size(); itree++)
    		{
                ...
                if(sample_list[isample].Contains("NPL") && systTree_list[itree] != "") {continue;}
                if(systTree_list[itree] != "" && sample_list[isample] != "tZq") {continue;}

    			tree = 0;
                TString tmp = systTree_list[itree];
    			if(tmp == "") {tmp = nominal_tree_name;}
    					...
    					for(int ivar=0; ivar<total_var_list.size(); ivar++)
    					{
                            ...
                            if(isPrivMC) //Private SMEFT samples, nominal -- Only used in Combine to extract yield parametrizations; also used in this code to plot SMEFT samples, etc.
                            {
                                ...
                                if(!makeHisto_inputVars && make_SMvsEFT_templates_plots)
                                {
                                    ...
                                    // StoreEachHistoBinIndividually(file_output, v3_TH1EFT_chan_syst_var[ichan][isyst][ivar], output_histo_name);
                                }
...
// #    #  ####
    			...
    			for(int isample = 0; isample < sample_list.size(); isample++)
    			{
    				...
    				else
    				{
                        ...
                        if(region != "" && !drawInputVars && !categorization_strategy) {histo_name+= "_" + region;}
    				...
    				if(draw_errors)
    				{
    					...
    					if(!use_combine_file) //In Combine file, already accounted in binError
    					{
    						for(int itree=0; itree<systTree_list.size(); itree++)
    						{
                                if((sample_list[isample] == "DATA" || sample_list[isample].Contains("NPL")) && systTree_list[itree] != "") {continue;}
                                if(systTree_list[itree] != "" && sample_list[isample] != "tZq") {continue;}
...
// #####  #    #   #   #    #
    		...
    		else
    		{
                ...
                if(region != "" && !drawInputVars && !categorization_strategy) {data_histo_name+= "_" + region;}
            ...
            if(superimpose_EFThist)
            {
                ...
                for(int ipoint=0; ipoint<v_EFT_points.size(); ipoint++)
                {
                    for(int isample=0; isample<v_EFT_samples.size(); isample++)
                    {
                        ...
                        th1eft_tmp->SetLineColor(icolor+3);
                        icolor++;
...
//--------------------------------------------
    ...
    if(type == "tZq")
    {
        ...
        v_samples.push_back("tZq"); v_groups.push_back("tZq (Central)"); v_colors.push_back(kBlue);
...
{
	cout<<endl<<FYEL("==> Merging some histograms (according to sample group) in file : ")<<filename<<endl;
        			...
        			for(int isyst=0; isyst<syst_list.size(); isyst++)
        			{
                        ...
                        for(int ibin=-1; ibin<n_singleBins+1; ibin++) //Convention (depending on NN_strategy): bin==-1 <-> merge full histogram; bin==0 <-> merge histo stored as single bin (counting exp.); bin>0 <-> merge corresponding template bin (individually)
                        {
            				...
            				for(int isample=0; isample<sample_list.size(); isample++)
            				{
                                ...
                                else if(systTree_list[itree] != "" && sample_list[isample] != "tZq") {continue;}
...
// void Make_ROC_fromTemplateFile(v_filepath, v_Filelabel, v_isTMVA_file, v_isTrainSample, region, v_processes, superimpose_allNodes_DNN, lumiYear, cuts)
{
    ...
    if(template_name == "NN") //Read NN info and weights
    {
        ...
        if(NNinfo_input_path == "") {return;} //MVA file not found
        if(!Extract_Values_From_NNInfoFile(NNinfo_input_path, var_list_NN, v_NN_nodeLabels, NN_inputLayerName, NN_outputLayerName, NN_iMaxNode, NN_nNodes, &NN_strategy)) {cout<<"ERROR: MVA info not found ! Abort ! "<<endl; return;}
    }
    ...
    histo_name_prefix+= + "_" + v_lumiYears[0]; //FIXME
